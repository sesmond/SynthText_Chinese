...
def main(viz=False):
    ...
    db_path = osp.join("data2", 'dset.h5')
    out_file = "temp1.h5"
    db = get_data(db_path)
    ...
    # end_idx =
    for i, im_name in enumerate(imnames):
        t1 = time.time()
        # imname = imnames[i]
        try:
            # get the image:
            img = Image.fromarray(db['image'][im_name][:])
            # get the pre-computed depth:
            #  there are 2 estimates of depth (represented as 2 "channels")
            #  here we are using the second one (in some cases it might be
            #  useful to use the other one):
            # TODO 这是什么？
            img_resize = img.resize(db['depth'][im_name].shape[1:])
            depth = db['depth'][im_name][:].T
            ...
            # get segmentation:
            seg = db['seg'][im_name][:].astype('float32')
            area = db['seg'][im_name].attrs['area']
            label = db['seg'][im_name].attrs['label']
            ...
            for ct in range(5):
                ...
                if len(res) > 0:
                    ...
                    add_res_to_db(im_name, res, out_db)
                    break
                else:
                    res = rv3.render_text(img, depth, seg, area, label,
                                          ninstance=INSTANCE_PER_IMAGE, viz=viz)
