...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        with open(font_model_path, 'rb') as f:
            temp_font_model = cp.load(f, encoding='bytes')
            font_model = {}
            for k, v in temp_font_model.items():
                if isinstance(k, bytes):
                    font_model[k.decode()] = v
                else:
                    font_model[k] = v
...
class TextSource(object):
    ...
    def is_good(self, txt, f=0.35):
        """
        T/F return : T if the lines in txt (a list of txt lines)
                     are "valid".
    ...
    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):
        ...
        while not np.all(self.is_good(lines, f)) and iter < niter:
            ...
            for i in range(nline):
                words = lines[i].split("。") #TODO 用什么拆分句子？ 句号可以吗
                ...
                while len(lines[i]) > nchar_max:  # chop-off characters from end:  TODO 这里为什么要这么做？
                    #TODO modify xmj 这里应该是变小，直接强制截断，没必要这么搞
    ...
    def sample(self, nline_max, nchar_max, kind='WORD'):
        result = self.fdict[kind](nline_max, nchar_max)
        print('sample_output:',kind, result)
        return result
        # self.fdict[kind](nline_max, nchar_max)
